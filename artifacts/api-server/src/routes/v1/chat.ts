    ...
    for await (const event of stream) {
      if (event.type === "message_start") {
        ...
        if (block.type === "thinking") {
          inThinking = true;
          // reasoning_content will be sent via content_block_delta
...
async function handleClaudeNonStream(
  ...
  // Compose message
  const fullContent: string | null = bodyText || null;
  ...
  // Surface thinking as a separate reasoning_content field (OpenAI-compatible)
...
router.post("/chat/completions", authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (stream) {
      ...
      if (isClaude) await handleClaudeStream(req, res, body, stats);
      else if (isGemini) await handleGeminiStream(req, res, body, stats);
      else if (isOpenRouter) await handleOpenRouterStream(req, res, body, stats);
      else await handleOpenAIStream(req, res, body, stats);
    } else {
      // Non-streaming — check cache first
      const cacheKey = hashRequest(body);
      ...
      if (cached) {
        ...
        if (isClaude) data = await handleClaudeNonStream(req, res, body, stats);
        else if (isGemini) data = await handleGeminiNonStream(req, res, body, stats);
        else if (isOpenRouter) data = await handleOpenRouterNonStream(req, res, body, stats);
        else data = await handleOpenAINonStream(req, res, body, stats);
